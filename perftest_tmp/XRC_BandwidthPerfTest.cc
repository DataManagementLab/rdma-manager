#include "XRC_BandwidthPerfTest.h"
...
mutex rdma::XRC_BandwidthPerfTest::waitLock;
condition_variable rdma::XRC_BandwidthPerfTest::waitCv;
bool rdma::XRC_BandwidthPerfTest::signaled;
rdma::TestOperation rdma::XRC_BandwidthPerfTest::testOperation;

rdma::XRC_BandwidthPerfClientThread::XRC_BandwidthPerfClientThread(std::vector<std::string>& rdma_addresses, std::string ownIpPort, std::string sequencerIpPort, size_t packet_size, int buffer_slots, size_t iterations_per_thread, size_t max_rdma_wr_per_thread, WriteMode write_mode) {
	Config::SEQUENCER_IP = rdma::Network::getAddressOfConnection(sequencerIpPort);
	this->m_client = new RDMAClient<ExReliableRDMA>(Config::RDMA_MEMSIZE, "XRC_BandwidthPerfTestClient");
	...
	for (size_t i = 0; i < m_rdma_addresses.size(); ++i) {
		...
		if(!m_client->connect(conn, nodeId)) {
			std::cerr << "XRC_BandwidthPerfThread::XRC_BandwidthPerfThread(): Could not connect to '" << conn << "'" << std::endl;
			throw invalid_argument("XRC_BandwidthPerfThread connection failed");
		}
...
rdma::XRC_BandwidthPerfClientThread::~XRC_BandwidthPerfClientThread() {
	for (size_t i = 0; i < m_rdma_addresses.size(); ++i) {
...
void rdma::XRC_BandwidthPerfClientThread::run() {
	rdma::PerfTest::global_barrier_client(m_client, m_addr); // global barrier

	unique_lock<mutex> lck(XRC_BandwidthPerfTest::waitLock); // local barrier
	if (!XRC_BandwidthPerfTest::signaled) {
		m_ready = true;
		XRC_BandwidthPerfTest::waitCv.wait(lck);
	}
	lck.unlock();
	m_ready = false;
	
	int sendCounter = 0, receiveCounter = 0, totalBudget = m_iterations_per_thread;
	uint32_t localBaseOffset = 0; //TODO: what should this be?(uint32_t)m_local_memory->getRootOffset();
	auto start = rdma::PerfTest::startTimer();
	switch(XRC_BandwidthPerfTest::testOperation){
			...
			switch(m_write_mode){
				case WRITE_MODE_NORMAL:
					for(size_t i = 0; i < m_iterations_per_thread; i++){
						...
						m_client->write(m_addr[connIdx], m_remOffsets[connIdx]+offset, m_client->getBuffer(offset), m_packet_size, signaled);
					}
					break;
				case WRITE_MODE_IMMEDIATE:
					for(size_t i = 0; i < m_iterations_per_thread; i+=2*m_max_rdma_wr_per_thread){
						...
						for(int j = 0; j < budgetR; j++){
							m_client->receiveSRQ(0, nullptr, 0);
						}
						for(int j = 0; j < budgetS; j++){
							size_t connIdx = i % m_rdma_addresses.size();
							int offset = (i % m_buffer_slots) * m_packet_size;
							m_client->writeImm(m_addr[(i+j) % m_rdma_addresses.size()], m_remOffsets[connIdx]+offset, m_client->getBuffer(offset), m_packet_size, localBaseOffset, (j+1)==budgetS);
						}

						bool do_poll = true;
						for(int j = 0; j < budgetR; j++){
							m_client->pollReceiveSRQ(0, m_addr[(fi+j) % m_rdma_addresses.size()], do_poll);
						}
					}
					break;
				default: throw invalid_argument("XRC_BandwidthPerfClientThread unknown write mode");
			}
		...
		case READ_OPERATION: // Read
			for(size_t i = 0; i < m_iterations_per_thread; i++){
				...
				bool signaled = (i == (m_iterations_per_thread - 1));
				int offset = (i % m_buffer_slots) * m_packet_size;
				m_client->read(m_addr[connIdx], m_remOffsets[connIdx] + offset, m_client->getBuffer(offset), m_packet_size, signaled);
			}
		...
		case SEND_RECEIVE_OPERATION: // Send & Receive
			...
			for(size_t i = 0; i < m_iterations_per_thread; i+=2*m_max_rdma_wr_per_thread){
				...
				for(int j = 0; j < budgetR; j++){
					...
					m_client->receiveSRQ(0, m_addr[(fi+j) % m_rdma_addresses.size()], m_client->getBuffer(receiveOffset), m_packet_size);
				}

				for(int j = 0; j < budgetS; j++){
					sendCounter = (sendCounter+1) % m_buffer_slots;
					int sendOffset = sendCounter * m_packet_size + m_memory_size_per_thread;
					m_client->send(m_addr[(i+j) % m_rdma_addresses.size()], m_client->getBuffer(sendOffset), m_packet_size, (j+1)==budgetS); // signaled: (j+1)==budget
				}
		...
		default: throw invalid_argument("XRC_BandwidthPerfClientThread unknown test mode");
	}
}



rdma::XRC_BandwidthPerfServerThread::XRC_BandwidthPerfServerThread(RDMAServer<ExReliableRDMA> *server, size_t packet_size, int buffer_slots, size_t iterations_per_thread, size_t max_rdma_wr_per_thread, WriteMode write_mode, int thread_id) {
...
rdma::XRC_BandwidthPerfServerThread::~XRC_BandwidthPerfServerThread() {
	// delete m_local_memory;  // implicitly deletes local allocs in RDMAServer
}

void rdma::XRC_BandwidthPerfServerThread::run() {
	unique_lock<mutex> lck(XRC_BandwidthPerfTest::waitLock);
	if (!XRC_BandwidthPerfTest::signaled) {
		m_ready = true;
		XRC_BandwidthPerfTest::waitCv.wait(lck);
	}
	...
	switch(XRC_BandwidthPerfTest::testOperation){
		case WRITE_OPERATION:
			if(m_write_mode == WRITE_MODE_IMMEDIATE){
				...
				i += budgetR;
				for(int j = 0; j < budgetR; j++){
					m_server->receiveSRQ(0, nullptr, 0);
				}
				...
				do {
					...
					totalBudget -= budgetS;
					for(int j = 0; j < budgetR; j++){
						m_server->pollReceiveSRQ(0, node_id, &remoteBaseOffset, do_poll);
					}
					...
					for(int j = 0; j < budgetS; j++){
						...
						m_server->writeImm(clientIds[m_thread_id], remoteOffset, m_server->getBuffer(sendOffset), m_packet_size, (uint32_t)(i+j), (j+1)==budgetS); // signaled: (j+1)==budget
					}
		...
		case SEND_RECEIVE_OPERATION:
			...
			for(int j = 0; j < budgetR; j++){
				receiveCounter = (receiveCounter+1) % m_buffer_slots;
				int receiveOffset = receiveCounter * m_packet_size;
				m_server->receiveSRQ(0, m_server->getBuffer(receiveOffset), m_packet_size);
			}
			...
			do {
				...
				size_t node_id;
				for(int j = 0; j < budgetR; j++){
					m_server->pollReceiveSRQ(0, node_id, do_poll);
				}
				...
				for(int j = 0; j < budgetS; j++){
					sendCounter = (sendCounter+1) % m_buffer_slots;
					int sendOffset = sendCounter * m_packet_size + m_memory_size_per_thread;
					m_server->send(clientIds[m_thread_id], m_server->getBuffer(sendOffset), m_packet_size, (j+1)==budgetS); // signaled: (j+1)==budget
				}
		...
		default: throw invalid_argument("XRC_BandwidthPerfServerThread unknown test mode");
	}
...
rdma::XRC_BandwidthPerfTest::XRC_BandwidthPerfTest(int testOperations, bool is_server, std::vector<std::string> rdma_addresses, int rdma_port, std::string ownIpPort, std::string sequencerIpPort, int local_gpu_index, int remote_gpu_index, int client_count, int thread_count, uint64_t packet_size, int buffer_slots, uint64_t iterations_per_thread, WriteMode write_mode) : PerfTest(testOperations){
	...
	this->m_iterations_per_thread = iterations_per_thread;
	this->m_write_mode = (write_mode!=WRITE_MODE_AUTO ? write_mode : rdma::XRC_BandwidthPerfTest::DEFAULT_WRITE_MODE);
	this->m_rdma_addresses = rdma_addresses;
}
rdma::XRC_BandwidthPerfTest::~XRC_BandwidthPerfTest(){
...
std::string rdma::XRC_BandwidthPerfTest::getTestParameters(bool forCSV){
...
std::string rdma::XRC_BandwidthPerfTest::getTestParameters(){
	return getTestParameters(false);
}

void rdma::XRC_BandwidthPerfTest::makeThreadsReady(TestOperation testOperation){
	XRC_BandwidthPerfTest::testOperation = testOperation;
	XRC_BandwidthPerfTest::signaled = false;
	if(m_is_server){
		// Server
		for(XRC_BandwidthPerfServerThread* perfThread : m_server_threads){ perfThread->start(); }
		for(XRC_BandwidthPerfServerThread* perfThread : m_server_threads){ while(!perfThread->ready()) usleep(Config::RDMA_SLEEP_INTERVAL); }
		rdma::PerfTest::global_barrier_server(m_server, (size_t)m_thread_count);

	} else {
		// Client
		for(XRC_BandwidthPerfClientThread* perfThread : m_client_threads){ perfThread->start(); }
		for(XRC_BandwidthPerfClientThread* perfThread : m_client_threads){ while(!perfThread->ready()) usleep(Config::RDMA_SLEEP_INTERVAL); }
	}
}

void rdma::XRC_BandwidthPerfTest::runThreads(){
	XRC_BandwidthPerfTest::signaled = false;
	unique_lock<mutex> lck(XRC_BandwidthPerfTest::waitLock);
	XRC_BandwidthPerfTest::waitCv.notify_all();
	XRC_BandwidthPerfTest::signaled = true;
	lck.unlock();
	for (size_t i = 0; i < m_server_threads.size(); i++) {
...
void rdma::XRC_BandwidthPerfTest::setupTest(){
	m_elapsedWriteMs = -1;
	m_elapsedReadMs = -1;
	m_elapsedSendMs = -1;
	...
	if(m_is_server){
		// Server
		m_server = new RDMAServer<ExReliableRDMA>("BandwidthTestRDMAServer", m_rdma_port);
		for (int thread_id = 0; thread_id < m_thread_count; thread_id++) {
			XRC_BandwidthPerfServerThread* perfThread = new XRC_BandwidthPerfServerThread(m_server, m_packet_size, m_buffer_slots, m_iterations_per_thread, max_rdma_wr_per_thread, m_write_mode, thread_id);
		...
		for (int i = 0; i < m_thread_count; i++) {
			XRC_BandwidthPerfClientThread* perfThread = new XRC_BandwidthPerfClientThread(m_rdma_addresses, m_ownIpPort, m_sequencerIpPort, m_packet_size, m_buffer_slots, m_iterations_per_thread, max_rdma_wr_per_thread, m_write_mode);
			m_client_threads.push_back(perfThread);
		}
	}
}

void rdma::XRC_BandwidthPerfTest::runTest(){
	if(m_is_server){
		// Server
		std::cout << "Starting server on '" << rdma::Config::getIP(rdma::Config::RDMA_INTERFACE) << ":" << m_rdma_port << "' . . ." << std::endl;
		if(!m_server->startServer()){
			std::cerr << "XRC_BandwidthPerfTest::runTest(): Could not start server" << std::endl;
			throw invalid_argument("XRC_BandwidthPerfTest server startup failed");
		} else {
...
std::string rdma::XRC_BandwidthPerfTest::getTestResults(std::string csvFileName, bool csvAddHeader){
	if(m_is_server){
		return "only client";
	} else {
		...
		*/
		...
		for(size_t i=0; i<m_client_threads.size(); i++){
			XRC_BandwidthPerfClientThread *thr = m_client_threads[i];
