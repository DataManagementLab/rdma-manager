...
DEFINE_string(test, "", "Tests: [bandwidth, xrcbandwidth, latency, operationscount, atomicsbandwidth, atomicslatency, atomicsoperationscount] OR MORE GRANULAR [write_bw, write_lat, write_ops, read_bw, read_lat, read_ops, send_bw, send_lat, send_ops, fetch_bw, fetch_lat, fetch_ops, swap_bw, swap_lat, swap_ops] (multiples separated by comma without space, not full word required) [Default bandwidth]");
...
DEFINE_string(interface, "", "Infiniband intrface");
DEFINE_int32(numa, -1, "Numa region");
...
enum TEST { BANDWIDTH_TEST=1, LATENCY_TEST=2, OPERATIONS_COUNT_TEST=3, ATOMICS_BANDWIDTH_TEST=4, ATOMICS_LATENCY_TEST=5, ATOMICS_OPERATIONS_COUNT_TEST=6, XRC_BANDWIDTH_TEST=7 };
...
int main(int argc, char *argv[]){
    ...
    rdma::Config *config = new rdma::Config(argv[0]);
    ...
    if(!FLAGS_interface.empty()) rdma::Config::RDMA_INTERFACE = FLAGS_interface;
    ...
    while(testIt != testNames.end()){
        ...
        if(std::string("bandwidth").find(testName) == 0){
            ...
            parse_op = false;
        } else if(std::string("xrcbandwidth").find(testName) == 0){
            test = XRC_BANDWIDTH_TEST;
            test_ops = (testOperations.find(test) != testOperations.end() ? testOperations[test] : 0);
            if(test_ops == 0){ count *= transfersizes.size() * packetsizes.size() * write_modes.size(); }
            parse_op = false;
    ...
    if(FLAGS_server){
        std::cout << "Own network ip: " << rdma::Network::getOwnAddress() << std::endl;
    ...
    rdma::Config::SEQUENCER_IP = rdma::Network::getAddressOfConnection(sequencerIpAddr);
                    ...
                    for(uint64_t &transfersize : transfersizes){
                        ...
                        for(rdma::WriteMode &write_mode : write_modes){
                            ...
                            for(uint64_t &packet_size : packetsizes){
                                ...
                                } else if(t == XRC_BANDWIDTH_TEST){
                                    // XRC Bandwidth Test
                                    testName = "XRC Bandwidth";
                                    test = new rdma::XRC_BandwidthPerfTest(test_ops, FLAGS_server, addresses, FLAGS_port, ownIpPort, sequencerIpAddr, local_gpu_index, remote_gpu_index, FLAGS_clients, thread_count, packet_size, buffer_slots, iterations_per_thread, write_mode);
