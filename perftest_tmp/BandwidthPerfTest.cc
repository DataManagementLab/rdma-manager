...
rdma::BandwidthPerfClientThread::BandwidthPerfClientThread(std::vector<std::string>& rdma_addresses, std::string ownIpPort, std::string sequencerIpPort, size_t packet_size, int buffer_slots, size_t iterations_per_thread, size_t max_rdma_wr_per_thread, WriteMode write_mode) {
	Config::SEQUENCER_IP = rdma::Network::getAddressOfConnection(sequencerIpPort);
	this->m_client = new RDMAClient<ReliableRDMA>(Config::RDMA_MEMSIZE, "BandwidthPerfTestClient");
				...
				case WRITE_MODE_IMMEDIATE:
					for(size_t i = 0; i < m_iterations_per_thread; i+=2*m_max_rdma_wr_per_thread){
						...
						for(int j = 0; j < budgetR; j++){
							m_client->receive(m_addr[(fi+j) % m_rdma_addresses.size()], nullptr, 0);
						}
		...
		case SEND_RECEIVE_OPERATION: // Send & Receive
			...
			for(size_t i = 0; i < m_iterations_per_thread; i+=2*m_max_rdma_wr_per_thread){
				...
				for(int j = 0; j < budgetR; j++){
					...
					m_client->receive(m_addr[(fi+j) % m_rdma_addresses.size()], m_client->getBuffer(receiveOffset), m_packet_size);
				}
	...
	switch(BandwidthPerfTest::testOperation){
		case WRITE_OPERATION:
			if(m_write_mode == WRITE_MODE_IMMEDIATE){
				...
				i += budgetR;
				for(int j = 0; j < budgetR; j++){
					m_server->receive(clientIds[m_thread_id], nullptr, 0);
				}
		...
		case SEND_RECEIVE_OPERATION:
			...
			for(int j = 0; j < budgetR; j++){
				receiveCounter = (receiveCounter+1) % m_buffer_slots;
				int receiveOffset = receiveCounter * m_packet_size;
				m_server->receive(clientIds[m_thread_id], m_server->getBuffer(receiveOffset), m_packet_size);
			}
...
void rdma::BandwidthPerfTest::setupTest(){
	...
	if(m_is_server){
		// Server
		m_server = new RDMAServer<ReliableRDMA>("BandwidthTestRDMAServer", m_rdma_port);
		...
		for (int i = 0; i < m_thread_count; i++) {
			BandwidthPerfClientThread* perfThread = new BandwidthPerfClientThread(m_rdma_addresses, m_ownIpPort, m_sequencerIpPort, m_packet_size, m_buffer_slots, m_iterations_per_thread, max_rdma_wr_per_thread, m_write_mode);
