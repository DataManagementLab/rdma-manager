...
public:
    ...
    static inline void global_barrier_client(rdma::RDMAClient<ExReliableRDMA> *client, const std::vector<NodeID> &connectionIDs){
        // Didn't use fetchAndAdd because memory polling is on GPUs very slow
        size_t node_id;
        std::atomic<bool> do_poll = true;
        for(const size_t &nodeID : connectionIDs){
            client->receiveSRQ(0, nodeID, client->getBuffer(nodeID), (size_t)1);
        }
        usleep(rdma::Config::PERFORMANCE_TEST_SERVER_TIME_ADVANTAGE); // let server first post its barrier
        for(const size_t &nodeID : connectionIDs){
            client->send(nodeID, client->getBuffer(nodeID), (size_t)1, do_poll);
        }
        size_t memIndex = 0xfffff;
        for(const size_t &nodeID : connectionIDs){
            client->pollReceiveSRQ(0, node_id, memIndex, do_poll);
        }
    };

    static inline void global_barrier_server(rdma::RDMAServer<ExReliableRDMA> *server, size_t expected_thread_count){
        // Didn't use fetchAndAdd because memory polling is on GPUs very slow
        std::vector<size_t> clientIds = server->getConnectedConnIDs();
        std::set<size_t> alreadyConnectedIds;
        size_t node_id;
        ...
        do {
            if(alreadyConnectedIds.size() < clientIds.size()){
                for(const size_t &nodeID : clientIds){
                    if(alreadyConnectedIds.find(nodeID) == alreadyConnectedIds.end()){
                        alreadyConnectedIds.insert(nodeID);
                        server->receiveSRQ(0, nodeID, server->getBuffer(nodeID), (size_t)1);
                    }
                }
            } else if(alreadyConnectedIds.size() < expected_thread_count) {
                usleep(rdma::Config::PERFORMANCE_TEST_SERVER_TIME_ADVANTAGE / 2); // let server first post its barrier
        ...
        // wait until all clients pinged and then pong back to all
        for(const size_t &nodeID : clientIds){
            server->pollReceiveSRQ(0, node_id, memIndex, do_poll);
        }
        // do_poll = false;
        for(const size_t &nodeID : clientIds){
            server->send(nodeID, server->getBuffer(nodeID), (size_t)1, do_poll);
        }
    };
