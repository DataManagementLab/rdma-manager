...
BaseRDMA::BaseRDMA(Memory *buffer) : BaseRDMA(buffer, false){}
BaseRDMA::BaseRDMA(Memory *buffer, bool passBufferOwnership) {
  m_buffer = buffer;
  m_buffer_owner = passBufferOwnership;
}
...
BaseRDMA::BaseRDMA(size_t mem_size, bool huge, int numaNode) : BaseRDMA(new Memory(mem_size, huge, numaNode), true){}
BaseRDMA::BaseRDMA(size_t mem_size, MEMORY_TYPE memType) : BaseRDMA(new Memory(mem_size, memType), true){}
...
void BaseRDMA::internalFree(const size_t &offset) { m_buffer->internalFree(offset); }
