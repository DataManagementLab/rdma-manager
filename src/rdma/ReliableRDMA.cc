...
void ReliableRDMA::destroyQPs()
{
    ...
    for (auto &kv : m_srqs)
    {
        ...
        {
            Logging::error(__FILE__, __LINE__, "ReliableRDMASRQ::destroyQPs: ibv_destroy_srq() failed ");
        }
    }
}

bool ReliableRDMA::write(const rdmaConnID rdmaConnID, size_t offset, const void *memAddr,
                                size_t size, bool signaled)
{
    return remoteAccess(rdmaConnID, offset, memAddr, size, signaled, true, IBV_WR_RDMA_WRITE);
}

bool ReliableRDMA::read(const rdmaConnID rdmaConnID, size_t offset, const void *memAddr,
                               size_t size, bool signaled)
{
    return remoteAccess(rdmaConnID, offset, memAddr, size, signaled, true, IBV_WR_RDMA_READ);
}

bool ReliableRDMA::requestRead(const rdmaConnID rdmaConnID, size_t offset, const void *memAddr,
                                size_t size)
{
    return remoteAccess(rdmaConnID, offset, memAddr, size, true, false, IBV_WR_RDMA_READ);
}

bool ReliableRDMA::fetchAndAdd(const rdmaConnID rdmaConnID, size_t offset, const void *memAddr,
                                      size_t size, bool signaled)
{
...
bool ReliableRDMA::send(const rdmaConnID rdmaConnID, const void *memAddr, size_t size,
                         bool signaled)
{
    DebugCode(
...
bool ReliableRDMA::receive(const rdmaConnID rdmaConnID, const void *memAddr, size_t size)
{
...
bool ReliableRDMA::modifyQPToRTR(struct ibv_qp *qp, uint32_t remote_qpn, uint16_t dlid,
                                  uint8_t *dgid)
{
    struct ibv_qp_attr attr;
...
bool ReliableRDMA::pollReceiveBatch(size_t srq_id, size_t &num_completed, bool &doPoll)
{
    int ne;
...
bool ReliableRDMA::pollReceiveSRQ(size_t srq_id, rdmaConnID &retRdmaConnID, bool &doPoll)
{
    int ne;
    struct ibv_wc wc;
...
{
    Logging::debug(__FILE__, __LINE__, "ReliableRDMA::createSharedReceiveQueue: Method Called");
...
bool ReliableRDMA::initQPForSRQWithSuppliedID(size_t srq_id, const rdmaConnID rdmaConnID)
{
    Logging::debug(__FILE__, __LINE__, "ReliableRDMA::initQP: Method Called with SRQID" + to_string(srq_id));
    struct ib_qp_t qp;
    //create queues
...
[[deprecated]]
bool ReliableRDMA::initQPForSRQ(size_t srq_id, rdmaConnID &retRdmaConnID)
{
    retRdmaConnID = nextConnKey(); //qp.qp->qp_num;
