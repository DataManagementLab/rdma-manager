...
public:
    ...
    virtual bool send(const rdmaConnID rdmaConnID, const void *memAddr, size_t size,
                      bool signaled) = 0;
    virtual bool receive(const rdmaConnID rdmaConnID, const void *memAddr, size_t size) = 0;
    virtual bool pollReceive(const rdmaConnID rdmaConnID, bool doPoll = true) = 0;
    // virtual bool pollReceive(const rdmaConnID rdmaConnID, uint32_t &ret_qp_num) = 0;
...
protected:
    ...
    inline void __attribute__((always_inline)) checkSignaled(bool &signaled, rdmaConnID rdmaConnID)
    {
        if (signaled)
            return;
