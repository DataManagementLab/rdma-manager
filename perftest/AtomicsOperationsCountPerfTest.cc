...
size_t rdma::AtomicsOperationsCountPerfTest::client_count;
size_t rdma::AtomicsOperationsCountPerfTest::thread_count;

rdma::AtomicsOperationsCountPerfClientThread::AtomicsOperationsCountPerfClientThread(Memory *memory, std::vector<std::string>& rdma_addresses, std::string ownIpPort, std::string sequencerIpPort, int buffer_slots, size_t iterations_per_thread) {
...
void rdma::AtomicsOperationsCountPerfClientThread::run() {
	...
	switch(AtomicsOperationsCountPerfTest::testOperation){
		case FETCH_ADD_OPERATION: // Fetch & Add
			...
			if(AtomicsOperationsCountPerfTest::client_count > 1) rdma::PerfTest::global_barrier_client(m_client, m_addr, false); // global end barrier if multiple nodes
...
void rdma::AtomicsOperationsCountPerfTest::makeThreadsReady(TestOperation testOperation){
	...
	if(m_is_server){
		...
		for(AtomicsOperationsCountPerfClientThread* perfThread : m_client_threads){ perfThread->start(); }
		for(AtomicsOperationsCountPerfClientThread* perfThread : m_client_threads){ while(!perfThread->ready()) usleep(Config::RDMA_SLEEP_INTERVAL); }
	}
}
...
std::string rdma::AtomicsOperationsCountPerfTest::getTestResults(std::string csvFileName, bool csvAddHeader){
	if(m_is_server){
		...
		*/
		...
		int64_t arrFetchAdd[thread_count];
		int64_t arrCompareSwap[thread_count];
		...
		std::sort(arrFetchAdd, arrFetchAdd + thread_count);
		std::sort(arrCompareSwap, arrCompareSwap + thread_count);
		medianFetchAdd = arrFetchAdd[(int)(thread_count/2)] / div;
		medianCompareSwap = arrCompareSwap[(int)(thread_count/2)] / div;
